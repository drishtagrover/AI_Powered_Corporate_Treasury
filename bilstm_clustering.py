...
# ✅ Define base model directory using Path
MODEL_DIR = Path(r"C:\Users\grove\financial_model\financial_model")
...
def predict_future_scores_and_cluster(df):
    ...
    if not (BILSTM_MODEL_FILE.exists() and SCALER_X_FILE.exists() and SCALER_Y_FILE.exists()):
        raise FileNotFoundError(
            f"⚠️ Missing model/scaler files in '{MODEL_DIR}'. Please verify all required files exist."
        )
